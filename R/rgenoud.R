...
genoud <- function(fn, nvars, max=FALSE, pop.size=1000, max.generations=100, wait.generations=10,
                   ...
                   transform=FALSE, debug=FALSE, cluster=FALSE, balance=FALSE, snow=FALSE, ...)
{
  ...
  if (is.logical(cluster)) {
    if (cluster==FALSE)  {
      ...
      stop("cluster option must be either FALSE, an object of the 'cluster' class 
           (from the 'parallel' package) or a list of machines so 'genoud' can create such an object")
    }
    }
  ...
  if(clustertrigger) {
    if(!snow) {
      ...
      message("sees that parallel does exist.")
      parallel.exists = require("snow")
    }
    if (!parallel.exists) {
  ...
  if(clustertrigger) {
    ...
    GENclusterExport <- function (cl, list) {
      ...
      for (name in list) {
        if(!snow) {
          ...
          message("Callint snow clusterCall")
    ...
    if (class(cluster)[1]=="SOCKcluster" | class(cluster)[1]=="PVMcluster" | 
          class(cluster)[1]=="spawnedMPIcluster" | class(cluster)[1]=="MPIcluster") {
      clustertrigger=1
      cl <- cluster
      ...
      if(!snow) {
        ...
        message("Making snow SOCK cluster")
        cl <- snow::makePSOCKcluster(cluster)
      }
      GENclusterExport(cl, "fn")        
      GENclusterExport(cl, "fn1")
    }
  ...
  fnMemoryMatrixEvaluate <- function(Memory, population, parms) {
    ...
    if (nrow(Memory) > 1)
      FIRSTTIME = FALSE
    nevaluate = 0
    mfunc <- function(pop, memory, ...) { # correct this to pass ...
      ...
      if (clustertrigger==0) {
        ...
        if (balance==TRUE) {
          ...
          if(!snow) {
            ...
            message("running snow clusterApplyLB")
          ...
          if(length(cl) > 1 ) {
            if(!snow) {
              ...
              message("running snow parRapply")
  ...
  if(is.matrix(starting.values)) starting.values <- t(starting.values)
                             ...
                             transform=FALSE, debug=FALSE, cluster=FALSE, balance=FALSE,  ...)
{
  ...
  fnMemoryMatrixEvaluate <- function(Memory, population, parms) {
    ...
    mfunc <- function(pop, memory , ...) { # corrected this
      f <- function(...) paste(..., sep=":")
      a2 <- do.call("f", as.data.frame(pop))
      b2 <- do.call("f", as.data.frame(memory))
      return(match(a2,b2))
