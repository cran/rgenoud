...
\Abstract{ This introduction to the \proglang{R} package \pkg{rgenoud}
  is a modified version of \cite{Mebane+Jr.+Sekhon:2009}, published in
  the \textit{Journal of Statistical Software}.  That version of the
  introduction contains higher resolution figures.
  
  Genoud is an \proglang{R} that combines evolutionary search
  algorithms with derivative-based (Newton or quasi-Newton) methods to
  solve difficult optimization problems. Genoud may also be used for
  optimization problems for which derivatives do not exist.  Genoud
  solves problems that are nonlinear or perhaps even discontinuous in
  the parameters of the function to be optimized.  When the function
  to be optimized (for example, a log-likelihood) is nonlinear in the
  model's parameters, the function will generally not be globally
  ...
  Multiple local optima may exist, so that there is no guarantee that
  a derivative-based method will converge to the global optimum.  On
  the other hand, algorithms that do not use derivative information
  (such as pure genetic algorithms) are for many problems needlessly
  poor at local hill climbing.  Most statistical problems are regular
  in a neighborhood of the solution.  Therefore, for some portion of
  the search space, derivative information is useful.  The function
...
developers.  Nine \proglang{R} packages currently rely upon
\pkg{rgenoud}: 
\pkg{anchors} \citep[analyzing survey data with
anchoring vignettes,][]{Wand+King:2009,WandKingLau2007,KingWand2007};
\pkg{boolean} \citep[boolean binary response models,][]{braumoeller2003,BraumoellerGoodrichKline2006}; 
\pkg{BARD} \citep[automated redistricting,][]{Altman+McDonald:2009};
\pkg{FAiR} \citep[factor analysis,][]{goodrich.fair}; 
\pkg{PKfit} \citep[data analysis in pharmacokinetics,][]{LeeLee2006};
\pkg{Matching} \citep[propensity score and multivariate matching,][]{Sekhon:2009,SekhonMatching}; 
...
\pkg{Synth} \citep[synthetic control group method for comparative case studies,][]{AbadieGardeazabal2003,synth2008}.
...
use of \pkg{rgenoud} are described in detail \citep{Sekhon:2009}.
...
\pkg{rgenoud}.\footnote{Lexical optimization is useful when there are
  ...
  first.  See the \code{lexical} option and \citet{Sekhon:2009} for
...
will resolve every difficulty'' (1981, 285).  In this article, we very briefly
...
positive value.  Nix and Vose (\citeyear{NixVose1992}; Vose
\citeyear{vose1993}) use a Markov chain model to show that in a GA where the
...
supplemental \proglang{R} file.  The vector \code{gradcheck} is true
for all elements except the one corresponding to function
6.\footnote{Also \code{gradcheck[7]==FALSE}.}
<<eval=false>>= 
...
\cite{Mebane+Jr.+Sekhon:2009}---also see
...
documentation).\footnote{Please see \cite{Mebane+Jr.+Sekhon:2009} or
